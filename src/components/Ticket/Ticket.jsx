...
export default function Ticket({ price, directFlight, returnFlight, carrier }) {
  ...
  const formRouteTime = (time, duration) => {
    ...
    return `${formatValue(takeoffTime.getHours())}:${formatValue(takeoffTime.getMinutes())} - ${formatValue(
      landingTime.getHours()
    )}:${formatValue(landingTime.getMinutes())}`
  }
    ...
    <div className={classes.ticket}>
      <header>
        ...
        <div className={classes.logo}>
          <img alt="logotype" src={`https://pics.avs.io/99/36/${carrier}.png`} />
        </div>
      </header>
      <div className={classes['direct-flight-field']}>
        <div className={classes.route}>
          <div className={classes.title}>
            {directFlight.origin} - {directFlight.destination}
          </div>
        ...
        <div className={classes.stops}>
          <div className={classes.title}>
            {directFlight.stops.length === 0 ? 'без' : directFlight.stops.length}{' '}
            {stopsCountCondition(directFlight.stops.length)}
          </div>
      ...
      <div className={classes['return-flight-field']}>
        ...
        <div className={classes.stops}>
          <div className={classes.title}>
            {returnFlight.stops.length === 0 ? 'без' : returnFlight.stops.length}{' '}
            {stopsCountCondition(returnFlight.stops.length)}
          </div>
